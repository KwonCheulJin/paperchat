"""
llama-server OpenAI 호환 클라이언트 — Generator Port 어댑터.
...
from app.domain.rag.ports import Generator
...
class LlamaServerGenerator(Generator):
    async def stream(
        ...
        async for token in stream_chat(messages, temperature, max_tokens, stop_sequences):
            yield token
    ...
    async def complete(
        ...
        return await chat_once(messages, temperature, max_tokens)
