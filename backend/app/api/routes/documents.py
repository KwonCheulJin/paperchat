...
from app.api.schemas.documents import DeleteResponse, DocumentInfo
from app.domain.document.service import delete_document, ingest_pdf, list_documents
...
async def ingest(
    ...
    async def _guarded():
        async for chunk in ingest_pdf(filename, content, folder):
            if await http_request.is_disconnected():
                logger.info("client_disconnected_ingest", filename=filename)
                return
            yield chunk
